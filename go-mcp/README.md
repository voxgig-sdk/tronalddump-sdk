...
[MCP](https://modelcontextprotocol.io) server exposing the Tronalddump SDK as
two agent tools — `tronalddump_list` and `tronalddump_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tronalddump-mcp)
make build

# 2. Provide credentials via the environment
export TRONALDDUMP_APIKEY=sk_live_xxx
...
```jsonc
// tronalddump_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// tronalddump_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tronalddump-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user tronalddump \
     -- "$PWD"/dist/*/tronalddump-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `tronalddump_list` and `tronalddump_load` tools now appear
   in new sessions. Ask the agent to *"list quote using tronalddump"*
   and it calls `tronalddump_list` with `{"entity":"quote"}`.
...
```sh
export TRONALDDUMP_APIKEY=sk_live_xxx            # API key
export TRONALDDUMP_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `tronalddump_list` tool
...
### Call the `tronalddump_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `tronalddump_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `tronalddump_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TRONALDDUMP_APIKEY` | API key sent with every request. |
| `TRONALDDUMP_BASE` | Optional override of the API base URL. |
