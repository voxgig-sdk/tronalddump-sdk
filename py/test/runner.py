# Tronalddump SDK test runner

from __future__ import annotations
import os
import json

from tronalddump_sdk.utility.voxgig_struct import voxgig_struct as vs


class TronalddumpTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TronalddumpTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TronalddumpTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TronalddumpTestRunner.getenv("TRONALDDUMP_TEST_LIVE")
        override = TronalddumpTestRunner.getenv("TRONALDDUMP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TronalddumpTestRunner.getenv(key)
        ...
        explain = TronalddumpTestRunner.getenv("TRONALDDUMP_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TRONALDDUMP_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TronalddumpTestRunner._test_control is not None:
            return TronalddumpTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TronalddumpTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TronalddumpTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TronalddumpTestRunner.load_test_control()
...
def load_env_local():
    TronalddumpTestRunner.load_env_local()


def env_override(m):
    return TronalddumpTestRunner.env_override(m)


def entity_data(v):
    return TronalddumpTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TronalddumpTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TronalddumpTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TronalddumpTestRunner.load_test_control()


def live_client_options():
    return TronalddumpTestRunner.live_client_options()


def live_delay_ms():
    return TronalddumpTestRunner.live_delay_ms()
