...
//     (\`new TronalddumpSDK(...)\` / \`TronalddumpSDK.test(...)\` become
//     \`TronalddumpSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"author":{"test01":{"id":"test01"}},"quote":{"test01":{"id":"test01"}},"source":{"test01":{"id":"test01"}},"tag":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TronalddumpSDK(...)\` and any
// \`TronalddumpSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TronalddumpSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TronalddumpSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
