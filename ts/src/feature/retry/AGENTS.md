# RetryFeature — Agent Guide

Automatic retry of transient failures with exponential backoff (v0.0.1).
...
runtime for the `retry` feature in the TypeScript target — do
...
- **Behaviour**: edit the runtime template under
  `.sdk/tm/ts/src/feature/retry/`, then regenerate.
