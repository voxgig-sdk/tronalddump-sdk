...
| Part | Path |
| --- | --- |
| Model definition | `.sdk/model/feature/ratelimit.aontu` (name, title, version, `config.options.active`, the `hook` map, per-language `deps`) |
| Registered in | `.sdk/model/feature/feature-index.aontu` (`@"ratelimit.aontu"`) |
...
- **Turn hooks on/off**: edit the `hook` map in
  `.sdk/model/feature/ratelimit.aontu` (`<Stage>: active: true|false`).
