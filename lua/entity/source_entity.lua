...
local SourceEntity = {}
SourceEntity.__index = SourceEntity


function SourceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SourceEntity)
  self._name = "source"
  self._client = client
...
function SourceEntity:get_name()
  return self._name
end


function SourceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SourceEntity.new(self._client, opts)
end
...
function SourceEntity:mark_deleted()
  self._deleted = true
end


function SourceEntity:deleted()
  return true == self._deleted
end


function SourceEntity:data_set(args)
  if args ~= nil then
...
function SourceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SourceEntity:match_set(args)
  if args ~= nil then
...
function SourceEntity:match_get()
...
function SourceEntity:stream(action, args, callopts)
...
---@param reqmatch SourceLoadMatch
---@param ctrl? table
---@return Source
---@return string? err
function SourceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SourceEntity:_run_op(ctx, post_done)
  local utility = self._utility
