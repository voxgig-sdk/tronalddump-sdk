...
module TronalddumpUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TronalddumpResponse.new({})
    ...
    if fetch_err
      ...
      response = TronalddumpResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TronalddumpResponse.new(fetched)
    else
