...
import (
	...
	omni "github.com/voxgig-sdk/tronalddump-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TronalddumpError); is {
				out := map[string]any{
					"name":    "TronalddumpError",
...
func omniCtx(arg any, client *sdk.TronalddumpSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TronalddumpSDK); is {
			client = live
