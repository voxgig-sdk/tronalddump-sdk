...
class SourceDirectTest < Minitest::Test
  def test_direct_load_source
    setup = source_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-source", setup[:live] ? "live" : "unit")
    if _should_skip
...
def source_direct_setup(mockres)
