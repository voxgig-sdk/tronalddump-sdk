...
class SourceEntity
  ...
  def make
    opts = @_entopts.dup
    SourceEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TronalddumpHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Source, Hash] the current Source data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TronalddumpHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Source fields)
  def match_get
  ...
  #
  # @param reqmatch [SourceLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Source.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Source, Hash] the loaded Source; raises TronalddumpError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TronalddumpHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
