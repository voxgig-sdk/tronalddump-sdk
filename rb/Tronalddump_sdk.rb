...
require_relative 'Tronalddump_types'


class TronalddumpSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TronalddumpHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TronalddumpHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TronalddumpFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TronalddumpUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TronalddumpHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TronalddumpHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TronalddumpHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => TronalddumpError.new(
        "#{op}_allow",
        "TronalddumpSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = TronalddumpHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TronalddumpError.new(
        "graphql_error", "TronalddumpSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = TronalddumpSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
