...
class FtCtrl extends TronalddumpControl
{
...
class FtClient
{
    ...
    private TronalddumpUtility $utility;

    public function __construct(TronalddumpUtility $utility, array $options)
    {
    ...
    public function get_utility(): TronalddumpUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TronalddumpContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TronalddumpContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TronalddumpContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TronalddumpConfig::make_feature($name);
    ...
    private static function buildUrl(TronalddumpSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TronalddumpError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TronalddumpResponse($fetched);
        }
    ...
    private function fail(TronalddumpContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TronalddumpContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
