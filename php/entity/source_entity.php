...
class SourceEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new SourceEntity($this->_client, $opts);
    }

    /**
     * @param Source|array $args Source data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TronalddumpHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Source|array The current Source data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Source fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TronalddumpHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Source fields).
     */
     ...
     *
     * @param SourceLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed SourceLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Source|array The loaded Source as an assoc-array at the
     *   SDK boundary; throws TronalddumpError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TronalddumpHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
